...
async def retrieve_event(
    ...
    logger.info(f"{user.username} is viewing event #{id} details.")
...
async def create_event(body: Event, user: TokenData = Depends(authenticate)) -> dict:
    body.creator = user.username
    logger.info(f"User [{user.username}] is creating an event.")
...
async def update_event(
    id: PydanticObjectId, body: EventUpdate, user: TokenData = Depends(authenticate)
) -> Event:
    logger.info(f"User [{user.username}] is updating event #{id}.")
    event = await event_database.get(id)
    if not event:
        logger.warning(f"\t The event #{id} NOT Found.")
        raise HTTPException(
...
async def delete_event(
    ...
    logger.info(f"User [{user.username}] is deleting event #{id}.")
    event = await event_database.get(id)
    if not event:
        logger.warning(f"\t The event #{id} NOT Found.")
        raise HTTPException(
